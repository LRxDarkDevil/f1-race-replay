...
def get_race_telemetry(session, session_type="R"):
    ...
    # 4.5. Prepare Fastest Lap Timeline
    fastest_lap_timeline = []

    # Get all valid laps
    for _, lap in session.laps.iterrows():
        if pd.isna(lap["LapTime"]) or pd.isna(lap["LapStartTime"]):
            continue

        # Time when the lap was COMPLETED relative to global_t_min
        lap_end_time = lap["LapStartTime"] + lap["LapTime"]
        lap_end_seconds = lap_end_time.total_seconds() - global_t_min

        lap_time_seconds = lap["LapTime"].total_seconds()

        # Helper to get abbreviation if needed, though session.laps['Driver'] usually has it
        driver = lap["Driver"]

        # Only include laps that finished after our start time
        # (Though technically a fastest lap set before the telemetry start is still valid as initial state)
        fastest_lap_timeline.append(
            {"time": lap_end_seconds, "lap_time": lap_time_seconds, "driver": driver}
        )

    # Sort by time of completion
    fastest_lap_timeline.sort(key=lambda x: x["time"])

    # Current Fastest Lap state
    # Initialize with any lap set before t=0 (if any)
    current_fastest_lap = {"time": float("inf"), "driver": None}

    # Check for laps completed before t=0
    pre_race_laps = [l for l in fastest_lap_timeline if l["time"] < 0]
    for l in pre_race_laps:
        if l["lap_time"] < current_fastest_lap["time"]:
            current_fastest_lap["time"] = l["lap_time"]
            current_fastest_lap["driver"] = l["driver"]

    # Filter timeline to only those >= 0 or handle index carefully
    ...
    for i in range(num_frames):
        ...
        # Update fastest lap holder based on current time t
        while fastest_lap_cursor < len(fastest_lap_timeline):
            record = fastest_lap_timeline[fastest_lap_cursor]
            if record["time"] <= t:
                # This lap has been completed
                if record["lap_time"] < current_fastest_lap["time"]:
                    current_fastest_lap["time"] = record["lap_time"]
                    current_fastest_lap["driver"] = record["driver"]
                fastest_lap_cursor += 1
            else:
                break
        ...
        for idx, car in enumerate(snapshot):
            ...
            frame_data[code] = {
                ...
                "brake": car["brake"],
                "fastest_lap": (code == current_fastest_lap["driver"]),
            }
