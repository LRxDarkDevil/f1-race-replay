...
class LegendComponent(BaseComponent):
    ...
    def draw(self, window):
        ...
        for i, lines in enumerate(self.lines):
            ...
            brackets = (
                lines[1] if isinstance(lines, tuple) and len(lines) > 2 else None
            )  # brackets only if icons exist
            icon_keys = (
                lines[2] if isinstance(lines, tuple) and len(lines) > 2 else None
            )  # icon keys
...
class WeatherComponent(BaseComponent):
    ...
    def draw(self, window):
        ...
        weather_lines = [
            ...
            (
                "Wind",
                f"{_fmt(info.get('wind_speed'), ' km/h')} {_format_wind_direction(info.get('wind_direction'))}",
                "wind",
            ),
        ...
        for idx, (label, value, icon_key) in enumerate(weather_lines):
            ...
            if weather_texture:
                ...
                arcade.draw_texture_rect(
                    rect=rect, texture=weather_texture, angle=0, alpha=255
                )
...
class LeaderboardComponent(BaseComponent):
    ...
    def draw(self, window):
        ...
        for i, (code, color, pos, progress_m) in enumerate(new_entries):
            ...
            arcade.Text(
                text, left_x, top_y, text_color, 16, anchor_x="left", anchor_y="top"
            ).draw()
            ...
            if tyre_texture:
                ...
                if pos.get("fastest_lap"):
                    fl_color = (177, 3, 252)  # Purple
                    fl_x = drs_dot_x - 10
                    fl_y = tyre_icon_y
                    arcade.Text(
                        "FL",
        ...
        if new_entries[0][2].get("lap", 0) == 1:
            arcade.Text(
                ...
                self.x,
                leaderboard_y - 30 - (len(new_entries) * self.row_height) - 20,
                arcade.color.YELLOW,
                12,
                anchor_x="left",
                anchor_y="top",
...
class QualifyingSegmentSelectorComponent(BaseComponent):
    ...
    def on_mouse_press(self, window, x: float, y: float, button: int, modifiers: int):
        ...
        if (
            close_btn_left <= x <= close_btn_right
            and close_btn_bottom <= y <= close_btn_top
        ):
...
class DriverInfoComponent(BaseComponent):
    ...
    def draw(self, window):
        ...
        for code in codes:
            ...
            self._draw_info_box(
                window, code, driver_pos, center_y, box_width, box_height
            )
    ...
    def _draw_info_box(self, window, code, driver_pos, center_y, box_width, box_height):
        ...
        drs_str, drs_color = (
            ("DRS: ON", arcade.color.GREEN)
            if drs_val in [10, 12, 14]
            else ("DRS: AVAIL", arcade.color.YELLOW)
            if drs_val == 8
            else ("DRS: OFF", arcade.color.GRAY)
        )
        arcade.Text(
        ...
        t_r, b_r = (
            max(0.0, min(1.0, thr / 100.0)),
            max(0.0, min(1.0, brk / 100.0 if brk > 1.0 else brk)),
        )
        ...
        arcade.draw_rect_filled(
            arcade.XYWH(r_center - 15, b_y + bar_h / 2, bar_w, bar_h),
...
class ControlsPopupComponent(BaseComponent):
    ...
    def draw(self, window):
        ...
        arcade.draw_rect_filled(
            arcade.XYWH(cx, header_cy, self.width, header_height), arcade.color.GRAY
        )
...
class SessionInfoComponent(BaseComponent):
    ...
    def draw(self, window):
        ...
        rect = arcade.XYWH(
            center_x, top_y - banner_height / 2, banner_width, banner_height
        )
...
class RaceProgressBarComponent(BaseComponent):
    ...
    def set_race_data(self, total_frames: int, total_laps: int, events: List[dict]):
        """
    ...
    def on_mouse_motion(self, window, x: float, y: float, dx: float, dy: float):
        ...
        if (
            ...
            for event in self._events:
                ...
                if (
                    dist < min_dist and dist < self._total_frames * 0.02
                ):  # Within 2% of timeline
                    min_dist = dist
                    nearest_event = event
...
class RaceControlsComponent(BaseComponent):
    ...
    def __init__(
        ...
        # Hover state
        self.hover_button = (
            None  # 'rewind/forward', 'play/pause', 'speed_increase', 'speed_decrease'
        )
    ...
    def draw(self, window):
        ...
        self._draw_speed_comp(
            forward_x + self.speed_container_offset,
            self.center_y,
            getattr(window, "playback_speed", 1.0),
        )

    def draw_hover_effect(
        self,
        button_name: str,
        x: float,
        y: float,
        ...
        if self.hover_button == button_name and getattr(
            self, f"{button_name}_rect", None
        ):
            arcade.draw_circle_outline(
                x,
                y,
                self.button_size // 2 + radius_offset,
                arcade.color.WHITE,
                border_width,
            )
        ...
        if self._flash_button == button_name and self._flash_timer > 0:
            ...
            arcade.draw_circle_outline(
                x,
                y,
                self.button_size // 2 + radius_offset + 2,
                flash_color,
                border_width + 1,
            )
    ...
    def _draw_speed_comp(self, x: float, y: float, speed: float):
        ...
        if "speed+" and "speed-" in self._control_textures:
            ...
            self.speed_decrease_rect = (
                x - button_offset - self.button_size // 2,
                y - self.button_size // 2,
                x - button_offset + self.button_size // 2,
                y + self.button_size // 2,
            )
            self.speed_increase_rect = (
                x + button_offset - self.button_size // 2,
                y - self.button_size // 2,
                x + button_offset + self.button_size // 2,
                y + self.button_size // 2,
            ...
            # Draw minus button
            arcade.draw_texture_rect(
                rect=rect_minus, texture=texture_minus, angle=0, alpha=255
            )
            ...
            # Draw plus button
            arcade.draw_texture_rect(
                rect=rect_plus, texture=texture_plus, angle=0, alpha=255
            )
        ...
        if self._point_in_rect(x, y, self.rewind_rect):
            ...
            if hasattr(window, "playback_speed"):
                ...
                if window.playback_speed < max(self.PLAYBACK_SPEEDS):
                    ...
                    window.playback_speed = self.PLAYBACK_SPEEDS[
                        min(current_index + 1, len(self.PLAYBACK_SPEEDS) - 1)
                    ]
    ...
    def _point_in_rect(
        self, x: float, y: float, rect: tuple[float, float, float, float] | None
    ) -> bool:
...
class QualifyingLapTimeComponent(BaseComponent):
    ...
    def draw(self, window):
        if not hasattr(window, "loaded_telemetry") or not window.loaded_telemetry:
            return
        sector_times = (
            window.loaded_telemetry.get("sector_times")
            if isinstance(window.loaded_telemetry, dict)
            else {}
        )
        if not sector_times:
            sector_times = {}
        ...
        arcade.Text(
            f"{driver_full_name}", self.x + 10, self.y - 30, driver_color, 16, bold=True
        ).draw()
        ...
        if (
            self.fastest_driver_sector_times
            and fastest_driver_full_name
            and fastest_driver_full_name != driver_full_name
        ):
        ...
        for sector_key, x_pos, sector_idx in sector_configs:
            ...
            if self.fastest_driver_sector_times:
                ...
                cumulative_fastest_time += (
                    fastest_sector_time if fastest_sector_time is not None else 0
                )
                delta_sector_time = (
                    sector_time - fastest_sector_time
                    if sector_time is not None and fastest_sector_time is not None
                    else None
                )
            ...
            elapsed_in_sector = (
                current_t if sector_idx == 0 else current_t - cumulative_time
            )
            ...
            # Check if sector is completed
                ...
                if sector_idx == 2 and fastest_sector_time is not None:
                    arcade.Text(
                        f"{formatted_fastest_sector_time}s",
                        self.x + 150,
                        self.y - 65,
                        arcade.color.LIGHT_GRAY,
                        13,
            ...
            arcade.Text(
                text, x_pos, self.y - 105, text_color, 12, anchor_x="center", bold=True
            ).draw()
    ...
    def draw_sector_labels(self, sector_times, current_t):
        ...
        s1_color = (
            arcade.color.GREEN
            if s1_time > 0 and current_t >= s1_time
        ...
        s2_color = (
            arcade.color.GREEN
            if s2_time > 0 and current_t >= s2_time
            else arcade.color.LIGHT_GRAY
        )
        ...
        s3_color = (
            arcade.color.GREEN
            if s3_time > 0 and current_t >= s3_time
            else arcade.color.LIGHT_GRAY
        )
        arcade.Text("S3", self.x + 200, self.y - 120, s3_color, 9, bold=True).draw()

    def show_delta_sector_times(
        self,
        sector_idx: int,
        sector_time: float,
        delta_sector_time: float | None,
        text_color: tuple,
    ):
        if (
            self._delta_sector == sector_idx
            and self._time_elapsed < 1.0
            and delta_sector_time is not None
        ):
...
def extract_race_events(
    ...
    for status in track_statuses:
        ...
        end_frame = (
            int(end_time * fps) if end_time else start_frame + 250
        )  # Default 10 seconds
...
def draw_finish_line(self, session_type="R"):
    ...
    if session_type == "Q" and len(self.inner_pts) > 0 and len(self.outer_pts) > 0:
        ...
        session_type == "R"
        and len(self.screen_inner_points) > 0
        and len(self.screen_outer_points) > 0
    ):
    ...
    if start_inner and start_outer:
        ...
        if length > 0:
            ...
            extended_inner = (
                start_inner[0] - extension * dx_norm,
                start_inner[1] - extension * dy_norm,
            )
            extended_outer = (
                start_outer[0] + extension * dx_norm,
